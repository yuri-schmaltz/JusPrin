...
void XMLCALL PrusaFileParser::start_element_handler(void* userData, const char* name, const char** attributes)
{
    ...
    if (prusa_parser != nullptr) {
        prusa_parser->_start_element_handler(name, attributes);
    }
}
...
void XMLCALL PrusaFileParser::characters_handler(void* userData, const XML_Char* s, int len)
{
    PrusaFileParser* prusa_parser = (PrusaFileParser*) userData;
    if (prusa_parser != nullptr) {
        prusa_parser->_characters_handler(s, len);
    }
}
...
void PrusaFileParser::_characters_handler(const XML_Char* s, int len)
{
    if (m_is_application_key) {
        ...
        if (!str.empty() && str.find("PrusaSlicer") != std::string::npos)
            m_from_prusa = true;
    }
}
...
void PrusaFileParser::_start_element_handler(const char* name, const char** attributes)
{
    if (::strcmp(name, "metadata") == 0) {
        ...
        if (!str_name.empty() && str_name.find("Application") != std::string::npos) {
            m_is_application_key = true;
        }
    }
}
...
class _3MF_Importer : public _3MF_Base
{
    struct Component
    {
        ...
        explicit Component(int object_id) : object_id(object_id), transform(Transform3d::Identity()) {}

        Component(int object_id, const Transform3d& transform) : object_id(object_id), transform(transform) {}
    };
...
private:
    ...
    bool _load_model_from_file(const std::string&         filename,
                               Model&                     model,
                               DynamicPrintConfig&        config,
                               ConfigSubstitutionContext& config_substitutions);
...
_3MF_Importer::~_3MF_Importer() { _destroy_xml_parser(); }
...
bool _3MF_Importer::_load_model_from_file(const std::string&         filename,
                                          Model&                     model,
                                          DynamicPrintConfig&        config,
                                          ConfigSubstitutionContext& config_substitutions)
{
            ...
            if (model_object->instances.size() > 1) {
                ...
                for (const IdToModelObjectMap::value_type& object : m_objects) {
                    if (object.second == int(i)) {
                        ...
                        if (!obj_geometry->second.has_mesh() && obj_geometry->second.volumes.empty()) {
                            // Mesh loading failed - report error with details
                            Slic3r::Logger::error("3MF", "Mesh loading failed for object '" + model_object->name +
                                                             "' (ID: " + std::to_string(object.first) + "). " +
                                                             "The mesh data may be corrupted or in an unsupported format.");
                            add_error("Unable to load mesh for object: " + model_object->name);
                            return false;
                        }
        ...
        res = mz_zip_reader_extract_file_to_callback(
            ...
            [](void* pOpaque, mz_uint64 file_ofs, const void* pBuf, size_t n) -> size_t {
                ...
                if (!XML_Parse(data->parser, (const char*) pBuf, (int) n, (file_ofs + n == data->stat.m_uncomp_size) ? 1 : 0) ||
                    ...
                    char error_buf[1024];
                    ::sprintf(error_buf, "Error (%s) while parsing '%s' at line %d", data->importer.parse_error_message(),
...
void _3MF_Importer::_extract_print_config_from_archive(mz_zip_archive&                 archive,
                                                       const mz_zip_archive_file_stat& stat,
                                                       DynamicPrintConfig&             config,
                                                       ConfigSubstitutionContext&      config_substitutions,
                                                       const std::string&              archive_filename)
            ...
            if (version == 0) {
                for (unsigned int i = 0; i < object_data_points.size(); i += 3)
                    sla_support_points.emplace_back(float(std::atof(object_data_points[i + 0].c_str())),
                                                    ...
                                                    float(std::atof(object_data_points[i + 2].c_str())), 0.4f, false);
            }
            if (version == 1) {
...
void _3MF_Importer::_handle_model_xml_characters(const XML_Char* s, int len) { m_curr_characters.append(s, len); }
...
bool _3MF_Importer::_handle_start_vertex(const char** attributes, unsigned int num_attributes)
{
    ...
    m_curr_object.geometry.vertices.emplace_back(m_unit_factor * get_attribute_value_float(attributes, num_attributes, X_ATTR),
...
bool _3MF_Importer::_handle_end_metadata()
{
    if (m_curr_metadata_name == SLIC3RPE_3MF_VERSION) {
        ...
        if (m_check_version && (m_version > VERSION_3MF_COMPATIBLE)) {
            ...
            // is not compatible.")); throw version_error(msg.c_str());
            const std::string msg = (boost::format(
                                         _(L("The selected 3mf file has been saved with a newer version of %1% and is not compatible."))) %
...
bool _3MF_Importer::_handle_start_config_metadata(const char** attributes, unsigned int num_attributes)
{
    ...
    std::vector<std::string> valid_keys = {"name",
                                           "volume_type",
                                           "matrix",
                                           "source_file",
                                           "source_object_id",
...
bool _3MF_Importer::_generate_volumes(ModelObject&                              object,
                                      const Geometry&                           geometry,
                                      const ObjectMetadata::VolumeMetadataList& volumes,
                                      ConfigSubstitutionContext&                config_substitutions)
{
    ...
    for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
        ...
        if (m_prusaslicer_generator_version && *m_prusaslicer_generator_version >= *Semver::parse("2.4.0-alpha1") &&
            *m_prusaslicer_generator_version < *Semver::parse("2.4.0-alpha3"))
            // PrusaSlicer 2.4.0-alpha2 contained a bug, where all vertices of a single object were saved for each volume the object
            // contained. Remove the vertices, that are not referenced by any face.
...
class _3MF_Exporter : public _3MF_Base
{
    struct BuildItem
    {
        ...
        BuildItem(unsigned int id, const Transform3d& transform, const bool printable) : id(id), transform(transform), printable(printable)
        {}
    };
    ...
    struct Offsets
    {
        ...
        Offsets(unsigned int first_vertex_id) : first_vertex_id(first_vertex_id), first_triangle_id(-1), last_triangle_id(-1) {}
    };
...
public:
    bool save_model_to_file(const std::string&        filename,
                            Model&                    model,
...
private:
    ...
    bool _add_object_to_model_stream(mz_zip_writer_staged_context& context,
...
bool _3MF_Exporter::save_model_to_file(const std::string&        filename,
                                       Model&                    model,
                                       const DynamicPrintConfig* config,
                                       bool                      fullpath_sources,
                                       const ThumbnailData*      thumbnail_data,
...
bool _3MF_Exporter::_save_model_to_file(const std::string&        filename,
                                        Model&                    model,
                                        const DynamicPrintConfig* config,
                                        const ThumbnailData*      thumbnail_data)
{
...
bool _3MF_Exporter::_add_thumbnail_file_to_archive(mz_zip_archive& archive, const ThumbnailData& thumbnail_data)
{
    ...
    void*  png_data = tdefl_write_image_to_png_file_in_memory_ex((const void*) thumbnail_data.pixels.data(), thumbnail_data.width,
                                                                 thumbnail_data.height, 4, &png_size, MZ_DEFAULT_LEVEL, 1);
    if (png_data != nullptr) {
...
bool _3MF_Exporter::_add_model_file_to_archive(const std::string& filename,
                                               mz_zip_archive&    archive,
                                               const Model&       model,
                                               IdToObjectDataMap& objects_data)
{
...

    {
        ...
        if (model.is_fdm_support_painted())
            stream << " <" << METADATA_TAG << " name=\"" << SLIC3RPE_FDM_SUPPORTS_PAINTING_VERSION << "\">" << FDM_SUPPORTS_PAINTING_VERSION
                   << "</" << METADATA_TAG << ">\n";

        if (model.is_seam_painted())
            stream << " <" << METADATA_TAG << " name=\"" << SLIC3RPE_SEAM_PAINTING_VERSION << "\">" << SEAM_PAINTING_VERSION << "</"
                   << METADATA_TAG << ">\n";
...
bool _3MF_Exporter::_add_object_to_model_stream(mz_zip_writer_staged_context& context,
                                                unsigned int&                 object_id,
                                                ModelObject&                  object,
                                                BuildItemsList&               build_items,
                                                VolumeToOffsetsMap&           volumes_offsets)
...
bool _3MF_Exporter::_add_mesh_to_object_stream(mz_zip_writer_staged_context& context,
                                               ModelObject&                  object,
                                               VolumeToOffsetsMap&           volumes_offsets)
{
...
bool _3MF_Exporter::_add_layer_height_profile_file_to_archive(mz_zip_archive& archive, Model& model)
{
    ...
    if (!out.empty()) {
        if (!mz_zip_writer_add_mem(&archive, LAYER_HEIGHTS_PROFILE_FILE.c_str(), (const void*) out.data(), out.length(),
        ...
        if (!sla_support_points.empty()) {
            ...
            for (size_t i = 0; i < sla_support_points.size(); ++i) {
                sprintf(buffer, (i == 0 ? "%f %f %f %f %f" : " %f %f %f %f %f"), sla_support_points[i].pos(0), sla_support_points[i].pos(1),
                        sla_support_points[i].pos(2), sla_support_points[i].head_front_radius, (float) sla_support_points[i].is_new_island);
                out += buffer;
            }
            out += "\n";
        }
    ...
    for (const ModelObject* object : model.objects) {
        ...
        if (!drain_holes.empty()) {
            ...
            for (size_t i = 0; i < drain_holes.size(); ++i)
                out += string_printf((i == 0 ? "%f %f %f %f %f %f %f %f" : " %f %f %f %f %f %f %f %f"), drain_holes[i].pos(0),
                                     drain_holes[i].pos(1), drain_holes[i].pos(2), drain_holes[i].normal(0), drain_holes[i].normal(1),
                                     drain_holes[i].normal(2), drain_holes[i].radius, drain_holes[i].height);
...
bool _3MF_Exporter::_add_model_config_file_to_archive(mz_zip_archive& archive, const Model& model, const IdToObjectDataMap& objects_data)
{
    ...
    for (const IdToObjectDataMap::value_type& obj_metadata : objects_data) {
        ...
        if (obj != nullptr) {
            ...
            if (!obj->name.empty())
                stream << "  <" << METADATA_TAG << " " << TYPE_ATTR << "=\"" << OBJECT_TYPE << "\" " << KEY_ATTR << "=\"name\" "
                       << VALUE_ATTR << "=\"" << xml_escape(obj->name) << "\"/>\n";

            // stores object's config data
            for (const std::string& key : obj->config.keys()) {
                stream << "  <" << METADATA_TAG << " " << TYPE_ATTR << "=\"" << OBJECT_TYPE << "\" " << KEY_ATTR << "=\"" << key << "\" "
                    ...
                    if (it != offsets.end()) {
                        ...
                        if (!volume->name.empty())
                            stream << "   <" << METADATA_TAG << " " << TYPE_ATTR << "=\"" << VOLUME_TYPE << "\" " << KEY_ATTR << "=\""
                                   << NAME_KEY << "\" " << VALUE_ATTR << "=\"" << xml_escape(volume->name) << "\"/>\n";
                        ...
                        stream << "   <" << METADATA_TAG << " " << TYPE_ATTR << "=\"" << VOLUME_TYPE << "\" " << KEY_ATTR << "=\""
                               << VOLUME_TYPE_KEY << "\" " << VALUE_ATTR << "=\"" << ModelVolume::type_to_string(volume->type())
                               << "\"/>\n";
                        ...
                        {
                            std::string input_file = xml_escape(m_fullpath_sources ?
...
bool store_3mf(
    const char* path, Model* model, const DynamicPrintConfig* config, bool fullpath_sources, const ThumbnailData* thumbnail_data, bool zip64)
{
