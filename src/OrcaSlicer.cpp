...
int CLI::run(int argc, char** argv)
{
    ...
    if (load_assemble_list.empty()) {
        for (const std::string& file : m_input_files) {
            ...
            try {
                ...
                // Validate that we don't have mixed FFF and SLA parameters
                // FFF uses parameters like layer_height, infill_density
                // SLA uses parameters like display_width, display_height, exposure_time
                bool has_fff_params = m_print_config.has("layer_height") || m_print_config.has("infill_density") ||
                                      m_print_config.has("support_material");

                bool has_sla_params = m_print_config.has("display_width") || m_print_config.has("display_height") ||
                                      m_print_config.has("exposure_time");

                if (has_fff_params && has_sla_params) {
                    boost::nowide::cerr << "ERROR: Configuration contains mixed FFF and SLA parameters." << std::endl;
                    boost::nowide::cerr << "Please use either FFF (layer_height, infill_density) or SLA (display_width, exposure_time) "
                                           "parameters, not both."
                                        << std::endl;
                    return CLI_INCOMPATIBLE_PARAMS_ERROR;
                }

                if (load_configs.size() > 1) {
                    ...
                    if (is_bbl_3mf) {
                        ...
                        if (!allow_newer_file && ((cli_ver.maj() != file_version.maj()) || (cli_ver.min() < file_version.min()))) {
                            BOOST_LOG_TRIVIAL(error)
                                << boost::format("Version Check: File Version %1% not supported by current cli version %2%") %
                                       file_version.to_string() % SoftFever_VERSION;
                        ...
                        if (normative_check) {
                            ...
                            if (postprocess_scripts) {
                                ...
                                if (postprocess_values.size() > 0) {
                                    BOOST_LOG_TRIVIAL(error)
                                        << boost::format("normative_check: postprocess not supported, array size %1%") %
                                               postprocess_values.size();
                                    record_exit_reson(outfile_dir, CLI_POSTPROCESS_NOT_SUPPORTED, 0,
                                                      cli_errors[CLI_POSTPROCESS_NOT_SUPPORTED], sliced_info);
                        ...
                        BOOST_LOG_TRIVIAL(info) << boost::format("current_printer_name %1%, current_process_name %2%") %
                                                       current_printer_name % current_process_name;
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("old extruder_clearance_height_to_rod %1%, extruder_clearance_height_to_lid "
        ...
        if (!custom_gcode_file.empty()) {
            ...
            try {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("load custom_gcode from file %1% success, store custom gcodes to plate %2%") %
                                               file % (plate_id + 1);
        ...
        for (auto const& file : load_configs) {
            ...
            if (config_type == "machine") {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format(
                                               "loaded machine config %1%, type %2%, name %3%, inherits %4%, printer_model_id %5%") %
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("loaded process config %1%, type %2%, name %3%, inherits %4%") % file %
                                               config_name % config_from % new_process_system_name;
            }
        ...
        if (use_first_fila_as_default) {
            ...
            for (int index = 0; index < load_filament_count; index++) {
                ...
                if (default_filament_file.empty() && !file.empty()) {
                    ...
                    BOOST_LOG_TRIVIAL(info) << boost::format("loaded default filament config %1%, type %2%, name %3%, inherits %4%") %
                                                   file % config_from % config_name % default_filament_inherit;
                    break;
                }
            }
        ...
        for (int index = 0; index < load_filament_count; index++) {
            ...
            if (!file.empty()) {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("loaded filament %1% from file %2%, type %3%, name %4%, inherits %5%") %
                                               (index + 1) % file % config_from % config_name % inherits;
            } else {
                if (use_first_fila_as_default) {
                    BOOST_LOG_TRIVIAL(info)
                        << __FUNCTION__
                        << boost::format(": load filament %1% from default, config name %2%, filament_id %3%, current_index %4%") %
        ...
        if (is_bbl_3mf && (load_filament_count > 0) && (load_filaments_set.size() == 1)) {
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("map all the filaments to the same one, load_filament_count %1%") %
                                           load_filament_count;
        }
            ...
            if (uptodate_configs.size() > 0) {
                for (auto const& file : uptodate_configs) {
                    ...
                    if (config_type == "machine") {
                        ...
                        upward_compatible_printers = config.option<ConfigOptionStrings>("upward_compatible_machine", true)->values;
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("load a machine config %1% from file %2%, upward_compatible_printers size is %3% ") %
                ...
                if (new_printer_name.empty() && !current_printer_system_name.empty()) {
                    ...
                    std::string system_printer_path = resources_dir() + "/profiles/BBL/machine_full/" + current_printer_system_name +
                                                      ".json";
                ...
                if (new_process_name.empty() && !current_process_system_name.empty()) {
                    // use the original printer name in 3mf
                    std::string system_process_path = resources_dir() + "/profiles/BBL/process_full/" + current_process_system_name +
                                                      ".json";
            ...
            if (load_filament_count == 0) {
                if (uptodate_filaments.size() > 0) {
                    ...
                    for (unsigned int index = 0; index < filament_count; index++) {
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("load a filament config %1% from file %2%, using uptodate_filaments index %3%") % config_name %
                                   file % index;
                    }
                } else {
                    current_index = 0;
                    for (int index = 0; index < current_filaments_system_name.size(); index++) {
                        ...
                        if (config_type != "filament") {
                            BOOST_LOG_TRIVIAL(error) << __FUNCTION__
                                                     << boost::format(": unknown config type %1% of file %2% in load-filaments") %
                                                            config_type % system_filament_path;
                            record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                            flush_and_exit(CLI_CONFIG_FILE_ERROR);
                        }
        ...
        BOOST_LOG_TRIVIAL(info)
            << boost::format(
                   "current printer inherits %1%, new printer inherits %2%, current process inherits %3%, new process inherits %4%") %
        ...
        for (int index = 0; index < upward_compatible_printers.size(); index++) {
            BOOST_LOG_TRIVIAL(info) << boost::format("index %1%, upward_compatible_printers %2%") % index %
                                           upward_compatible_printers[index];
        }
        if (!new_printer_name.empty()) {
            if (!new_process_name.empty()) {
                for (int index = 0; index < new_print_compatible_printers.size(); index++) {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format(
                                               ...
                                               new_printer_name % new_printer_system_name % current_process_name %
        ...
        if (!process_compatible && !new_printer_name.empty() && !current_printer_name.empty() &&
            (new_printer_name != current_printer_name)) {
            ...
            if (upward_compatible_printers.size() > 0) {
                ...
                if (!process_compatible) {
                    BOOST_LOG_TRIVIAL(error)
                        << __FUNCTION__
                        << boost::format(" %1% : current 3mf file not support the new printer %2%, new_printer_system_name %3%") %
            ...
            /*else {
                BOOST_LOG_TRIVIAL(error) <<__FUNCTION__ << boost::format(" %1%: current 3mf file not support upward_compatible_printers, can
            not change machine preset.")%__LINE__; record_exit_reson(outfile_dir, CLI_3MF_NOT_SUPPORT_MACHINE_CHANGE, 0,
        ...
        if (is_bbl_3mf && machine_switch) {
            // we need to update the compatible printer and create a new process here, or if we load the 3mf in studio, the process preset
            // can not be loaded as not compatible
        ...
        auto update_full_config = [](DynamicPrintConfig& full_config, const DynamicPrintConfig& config,
                                     std::set<std::string>& diff_key_sets, bool update_all = false, bool skip_gcodes = false) {
        ...
        std::vector<std::string>& different_settings = m_print_config.option<ConfigOptionStrings>("different_settings_to_system", true)
                                                           ->values;
        ...
        if (!new_printer_name.empty() || up_config_to_date) {
            ...
            if (new_printer_name.empty()) {
                ...
                if (new_printer_name != current_printer_name) {
                    ...
                    BOOST_LOG_TRIVIAL(info)
                        << boost::format(
                               "check printer cli safe params, current_printer_name %1%, new_printer_name %2%, printer_model %3%") %
                    ...
                    if (!fs::exists(directory_path)) {
                        ...
                        try {
                            ...
                            if (root_json.contains("printer")) {
                                ...
                                if (!printer_model.empty() && printer_json.contains(printer_model)) {
                                    ...
                                    if (printer_model_json.contains("machine_limits")) {
                                            ...
                                            if (option) {
                                                ...
                                                for (unsigned int index = 0; index < array_count; index++) {
                                                    if ((index < new_array_count) && new_option.values[index] != 0.f &&
                                                        ...
                                                        BOOST_LOG_TRIVIAL(info)
                                                            << boost::format("set key %1%  index %2%, from %3%  to %4%") % key % index %
                                        ...
                                        BOOST_LOG_TRIVIAL(info)
                                            << boost::format("can not find machine_limits for printer %1% in cli_config.json") %
                                                   printer_model;
                                    }
                                } else {
                            ...
                            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": parse file " << cli_config_file
                                                     << " got a generic exception, reason = " << err.what();
                        }
                    }
                }
        ...
        if ((load_filament_count > 0) || (up_config_to_date)) {
            for (int index = 0; index < load_filaments_config.size(); index++) {
                ...
                int                   diff_keys_size = different_keys_set.size();
                BOOST_LOG_TRIVIAL(info)
                    << boost::format("update filament %1%'s config to newest, different size %2%, name %3%, different_settings %4%") %
                ...
                for (const t_config_option_key& opt_key : config.keys()) {
                    ...
                    if (source_opt->is_scalar()) {
                        ...
                        if (opt == nullptr) {
                            ...
                            BOOST_LOG_TRIVIAL(error) << boost::format("can not create option %1% to config, from filament %2%: %3%") %
                                                            opt_key % filament_index % load_filaments_name[index];
                            record_exit_reson(outfile_dir, CLI_CONFIG_FILE_ERROR, 0, cli_errors[CLI_CONFIG_FILE_ERROR], sliced_info);
                            flush_and_exit(CLI_CONFIG_FILE_ERROR);
                        }
        ...
        if ((!project_filament_colors_option || (project_filament_colors_option->values.size() == 0)) && selected_filament_colors_option) {
            BOOST_LOG_TRIVIAL(info) << boost::format(
                "initial project_filament_colors is null, create it due to filament_colour set in cli");
            ...
            if (selected_filament_colors_option) {
                ...
                if (disable_wipe_tower_after_mapping) {
                    ...
                    if (filament_color_set.size() > 1) {
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("different filament colours,  switch disable_wipe_tower_after_mapping back to false");
                    } else {
                        BOOST_LOG_TRIVIAL(warning)
                            << boost::format("only %1% filament colour,  finally set disable_wipe_tower_after_mapping to true") %
                    ...
                    for (int to_idx = 0; to_idx < project_filament_count; to_idx++) {
                        ...
                        if (from_idx == to_idx) {
                            ...
                            if (is_to_support) {
                                ...
                                // BOOST_LOG_TRIVIAL(info) << boost::format("src_idx %1%, src color %2%, dst idex %3%, dst color
                                // %4%")%from_idx %from_color %to_idx %to_color; BOOST_LOG_TRIVIAL(info) << boost::format("src_rgba
                                // {%1%,%2%,%3%,%4%} dst_rgba
        ...
        if (skip_modified_gcodes) {
            ...
            for (gcodes_iter = m_models[0].plates_custom_gcodes.begin(); gcodes_iter != m_models[0].plates_custom_gcodes.end();
                 gcodes_iter++) {
        ...
        if (is_bbl_3mf && (old_printable_width > 0) && (old_printable_depth > 0) && (old_printable_height > 0)) {
            ...
            if ((old_printable_width > current_printable_width) || (old_printable_depth > current_printable_depth) ||
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("old printable size {%1%, %2%, %3%} is smaller than new printable size {%4%, %5%, "
                                                         "%6%}, need to center the model") %
                                               old_printable_width % old_printable_depth % old_printable_height % current_printable_width %
                                               current_printable_depth % current_printable_height;
                shrink_to_new_bed = 1;
            } else {
        ...
        if (m_models.size() > 0) {
            BOOST_LOG_TRIVIAL(info) << boost::format("translate_old %1%, shrink_to_new_bed %2%, old bed size {%3%, %4%, %5%}") %
                                           translate_old % shrink_to_new_bed % old_printable_width % old_printable_depth %
        ...
        auto check_plate_wipe_tower = [get_print_sequence, is_smooth_timelapse](Slic3r::GUI::PartPlate* plate, int plate_index,
            ...
            BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%, object bbox: min {%2%, %3%, %4%} - max {%5%, %6%, %7%}") %
                                           (plate_index + 1) % plate_obj_size_info.obj_bbox.min.x() % plate_obj_size_info.obj_bbox.min.y() %
            ...
            Vec3d wipe_tower_size          = plate->estimate_wipe_tower_size(print_config, plate_obj_size_info.wipe_width, wipe_volume,
                                                                             filaments_cnt);
            ...
            if (shrink_to_new_bed > 0) {
                ...
                for (int index = 0; index < plate_count; index++) {
                    ...
                    if (shrink_to_new_bed == 1) {
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("shrink_to_new_bed 1, plate %1%, cur_origin: {%2%, %3%}, new_origin: {%4%, "
                                             "%5%}, cur_center {%6%, %7%} new_center {%8%, %9%}") %
                                   (index + 1) % cur_origin(0) % cur_origin(1) % new_origin(0) % new_origin(1) % cur_center(0) %
                                   cur_center(1) % new_center(0) % new_center(1);
                    } else {
                        // center the object
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("shrink_to_new_bed 2, plate %1%, new_origin: {%2%, %3%}, new_center: {%4%, "
                                             "%5%}, obj bbox(including wipe tower) min {%6%, %7%} max {%8%, %9%}") %
                                   (index + 1) % new_origin(0) % new_origin(1) % new_center(0) % new_center(1) % bbox.min(0) % bbox.min(1) %
                                   bbox.max(0) % bbox.max(1);
                    }
                    offset(2) = 0.f;
                    BOOST_LOG_TRIVIAL(info) << boost::format(
                                                   "shrink_to_new_bed %1%, plate %2% translate offset: {%3%, %4%} wipe_offset {%5%, %6%}") %
                                                   shrink_to_new_bed % (index + 1) % offset[0] % offset[1] % wipe_offset[0] %
                                                   wipe_offset[1];
                    cur_plate->translate_all_instance(offset);

                    if (wipe_x_option) {
                        BOOST_LOG_TRIVIAL(info) << boost::format("shrink_to_new_bed %4%, plate %1%: wipe tower src: {%2%, %3%}") %
        ...
        if (plate_data_src.size() > 0) {
            ...
            for (int index = 0; index < plate_count; index++) {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%,  has_wipe_tower %2%, wipe_x %3%, wipe_y %4%, width %5%, depth %6%") %
                                               (index + 1) % plate_obj_size_infos[index].has_wipe_tower %
                                               plate_obj_size_infos[index].wipe_x % plate_obj_size_infos[index].wipe_y %
                                               plate_obj_size_infos[index].wipe_width % plate_obj_size_infos[index].wipe_depth;
            }
        ...
        if (downward_check) {
            ...
            for (auto const& file : downward_settings) {
                ...
                BOOST_LOG_TRIVIAL(info) << boost::format(
                                               "downward_check: printable size{%1%,%2%, %3%}, exclude area{%4%, %5%: %6% x %7%}") %
                                               printer_plate.printable_width % printer_plate.printable_depth %
                                               printer_plate.printable_height % printer_plate.exclude_x % printer_plate.exclude_y %
                                               printer_plate.exclude_width % printer_plate.exclude_depth;
        ...
        if (downward_check_size > 0) {
            ...
            for (int index = 0; index < plate_count; index++) {
                ...
                for (int index2 = 0; index2 < downward_check_size; index2++) {
                    ...
                    if ((size.z() > plate_info.printable_height) || (size.y() > plate_info.printable_depth) ||
                        ...
                        BOOST_LOG_TRIVIAL(info) << boost::format("plate %1%, downward_check index %2%, name %3%, bbox {%4%, %5%, %6%} "
                                                                 "exceeds printer size {%7%, %8%, %9%}") %
                                                       (index + 1) % (index2 + 1) % plate_info.printer_name % size.x() % size.y() %
                                                       size.z() % plate_info.printable_width % plate_info.printable_depth %
                                                       plate_info.printable_height;
                    ...
                    if (plate_info.exclude_width > 0) {
                        ...
                        if ((size.x() > real_width) && (size.y() > real_depth)) {
                            BOOST_LOG_TRIVIAL(info)
                                << boost::format("plate %1%, downward_check index %2%, name %3%, bbox {%4%, %5%} exceeds "
                                                 "real size without exclude_area {%6%, %7%}") %
                                       (index + 1) % (index2 + 1) % plate_info.printer_name % size.x() % size.y() % real_width % real_depth;
                            downward_check_status[index2] = true;
                            failed_count++;
                            continue;
                        }
            ...
            if (failed_count < downward_check_size) {
                // has success ones
                BOOST_LOG_TRIVIAL(info) << boost::format("downward_check: downward_check_size %1%, failed_count %2%") %
                                               downward_check_size % failed_count;
                for (int index2 = 0; index2 < downward_check_size; index2++) {
                    if (downward_check_status[index2])
                        continue;
                    printer_plate_info_t& plate_info = downward_check_printers[index2];
        ...
        for (auto& model : m_models) {
            for (ModelObject* o : model.objects) {
                if (orients_requirement[o->id().id]) {
                    BOOST_LOG_TRIVIAL(info) << "Before process command, Orient object, name=" << o->name << ",id=" << o->id().id
                                            << std::endl;
        ...
        if ((plate_to_slice < 0) || (plate_to_slice > partplate_list.get_plate_count())) {
            BOOST_LOG_TRIVIAL(error) << boost::format("invalid plate id %1%, total %2%") % plate_to_slice %
                                            partplate_list.get_plate_count();
            record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, cli_errors[CLI_INVALID_PARAMS], sliced_info);
            flush_and_exit(CLI_INVALID_PARAMS);
            ...
            if (duplicate_count > 0) {
                ...
                if ((spiral_vase != nullptr) && spiral_vase->value) {
                    ...
                    if (!is_seq_print_for_curr_plate) {
                        BOOST_LOG_TRIVIAL(warning)
                            << boost::format("Spiral mode can not be duplicated under by-object print, skip duplicate");
                        duplicate_count = 0;
                    }
                }
            }
            ...
            for (int index = 0; index < partplate_list.get_plate_count(); index++) {
                ...
                if (plate_data_src.size() > index) {
                    ...
                    if (!plate_data_src[index]->top_file.empty()) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("Plate %1%: clear loaded top_thumbnail %2%.") % (index + 1) % plate_data_src[index]->top_file;
                ...
                if (assemble_plate.need_arrange) {
                    ...
                    for (size_t oidx = 0; oidx < plate_obj_count; ++oidx) {
                        ...
                        for (size_t inst_idx = 0; inst_idx < mo->instances.size(); ++inst_idx) {
                            ...
                            BOOST_LOG_TRIVIAL(debug) << boost::format("plate %1%: add object %2%  object index %3%, into selected") %
                                                            (i + 1) % ap.name % (oidx + 1);
                        }
                    }
                    ...
                    if (!arrange_cfg.is_seq_print && assemble_plate.filaments_count > 1) {
                        ...
                        ArrangePolygon wipe_tower_ap = cur_plate->estimate_wipe_tower_polygon(m_print_config, i,
                                                                                              assemble_plate.filaments_count, true);
                ...
                while (!finished_arrange) {
                        ...
                        if (!first_run) {
                            ...
                            if (plate_to_slice > 0) {
                                ...
                                if (wipe_x_option && (wipe_x_option->size() > (plate_to_slice - 1))) {
                                    ...
                                    BOOST_LOG_TRIVIAL(info) << boost::format("%1%, plate_to_slice %2%, orig_wipe_x=%3%") % __LINE__ %
                                                                   plate_to_slice % orig_wipe_x;
                                }
                                if (wipe_y_option && (wipe_y_option->size() > (plate_to_slice - 1))) {
                                    orig_wipe_y = wipe_y_option->get_at(plate_to_slice - 1);
                                    BOOST_LOG_TRIVIAL(info) << boost::format("%1%, plate_to_slice %2%, orig_wipe_y=%3%") % __LINE__ %
                                                                   plate_to_slice % orig_wipe_y;
                                }
                            }
                        }
                    ...
                    if ((duplicate_count == 0) && (plate_to_slice == 0)) {
                        ...
                        if (used_filament_set.size() > 0) {
                            ...
                            BOOST_LOG_TRIVIAL(info)
                                << boost::format("prime_tower_width %1% wipe_tower_rotation_angle %2% prime_volume %3%") %
                                       width_option->value % rotation_angle_option->value % volume_option->value;
                        ...
                        for (size_t oidx = 0; oidx < model.objects.size(); ++oidx) {
                            ...
                            for (size_t inst_idx = 0; inst_idx < mo->instances.size(); ++inst_idx) {
                                ModelInstance* minst    = mo->instances[inst_idx];
                                bool           in_plate = cur_plate->contain_instance(oidx, inst_idx) ||
                                                cur_plate->intersect_instance(oidx, inst_idx);
                                ArrangePolygon ap = get_instance_arrange_poly(minst, m_print_config);

                                ArrangePolygons& cont = mo->instances[inst_idx]->printable ? (in_plate ? selected : unselected) :
                                                                                             unprintable;
                        ...
                        if (m_print_config.has("wipe_tower_x") &&
                            (is_smooth_timelapse || !arrange_cfg.is_seq_print || (selected.size() <= 1))) {
                            float x;
                            float y;
                            if (duplicate_count > 0) {
                                auto printer_structure_opt = m_print_config.option<ConfigOptionEnum<PrinterStructure>>("printer_structure");
                                ...
                                // keep the original
                                x = dynamic_cast<const ConfigOptionFloats*>(m_print_config.option("wipe_tower_x"))
                                        ->get_at(plate_to_slice - 1);
                                y = dynamic_cast<const ConfigOptionFloats*>(m_print_config.option("wipe_tower_y"))
                                        ->get_at(plate_to_slice - 1);
                            }
                    ...
                    if (duplicate_count == 0) {
                        if (plate_to_slice > 0) {
                            ...
                            for (ArrangePolygon& ap : selected) {
                                ...
                                if (ap.bed_idx != (plate_to_slice - 1)) {
                                    BOOST_LOG_TRIVIAL(error)
                                        << __FUNCTION__
                                        << boost::format(":arrange failed: ap.name %1% ap.bed_idx %2%, plate index %3%") % ap.name %
                                               ap.bed_idx % (plate_to_slice - 1);
                        ...
                        // Apply the arrange result to unselected objects(due to the sukodu-style column changes, the position of unselected
                        ...
                        for (ArrangePolygon& ap : selected) {
                            ...
                            if (ap.bed_idx != (plate_to_slice - 1)) {
                                ...
                                if (!duplicate_single_object) {
                                    BOOST_LOG_TRIVIAL(warning)
                                        << boost::format("arrange failed when duplicate multiple objects at count %1%, "
                                        ...
                                        if (skip_maps.find(i->loaded_id) != skip_maps.end()) {
                                            ...
                                            if (i->print_volume_state == ModelInstancePVS_Inside) {
                                                ...
                                                if (plate_data_src.size() > index) {
                                                    ...
                                                    if (!plate_data_src[index]->no_light_thumbnail_file.empty()) {
                                                        BOOST_LOG_TRIVIAL(info)
                                                            << boost::format("Plate %1%: clear loaded no_light_thumbnail %2%.") %
                                                                   (index + 1) % plate_data_src[index]->no_light_thumbnail_file;
                                                        plate_data_src[index]->no_light_thumbnail_file.clear();
                                                    }
                                                    if (!plate_data_src[index]->top_file.empty()) {
                                                        BOOST_LOG_TRIVIAL(info)
                                                            << boost::format("Plate %1%: clear loaded top_thumbnail %2%.") % (index + 1) %
                                            ...
                                            for (const ModelVolume* vol : model_object->volumes) {
                                                if (vol->is_model_part()) {
                                                    ...
                                                    if ((max_triangle_count_per_plate != 0) &&
                                                        (triangle_count > max_triangle_count_per_plate)) {
                                                        BOOST_LOG_TRIVIAL(error)
                            ...
                            if (print->empty()) {
                                ...
                                try {
                                    ...
                                    if (load_slicedata) {
                                        ...
                                        BOOST_LOG_TRIVIAL(info)
                                            << "print::process: first time_using_cache is " << time_using_cache << " secs.";
                                    }
                                    if (printer_technology == ptFFF) {
                                            ...
                                            for (unsigned int i = 0; i < g_slicing_warnings.size(); i++) {
                                                ...
                                                if ((status.warning_step != -1) &&
                                                    ...
                                                    if (status.warning_level == PrintStateBase::WarningLevel::NON_CRITICAL) {
                                                        BOOST_LOG_TRIVIAL(warning)
                                                            << "plate " << index + 1
                                                            << ": found NON_CRITICAL slicing warnings: " << status.text << std::endl;
                                                    } else {
                                                        BOOST_LOG_TRIVIAL(warning)
                                        ...
                                        // m_fff_print->export_gcode(m_temp_output_path, m_gcode_result, [this](const ThumbnailsParams&
                                        // params) { return this->render_thumbnails(params); });
                                    } /* else {
                                         outfile = sla_print.output_filepath(outfile);
                                         // We need to finalize the filename beforehand because the export function sets the filename inside
                                     the zip metadata outfile_final = sla_print.print_statistics().finalize_output_path(outfile);
        ...
        if (export_to_3mf) {
            ...
            if (sliced_plate == -1) {
                ...
                for (int i = 0; i < plate_data_list.size(); i++) {
                    if (skip_useless_pick && (plate_object_count[i] == 1)) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("only has 1 object,  set plate %1%'s is_label_object_enabled from %2% to false") % (i + 1) %
            ...
            for (int i = 0; i < plate_data_list.size(); i++) {
                ...
                if (!plate_data->plate_thumbnail.is_valid()) {
                    ...
                    BOOST_LOG_TRIVIAL(info)
                        << boost::format("thumbnails stage: plate %1%'s thumbnail data is invalid, check the file %2% exist or not") %
                               (i + 1) % plate_data->thumbnail_file;
                    if (plate_data->thumbnail_file.empty() || (!boost::filesystem::exists(plate_data->thumbnail_file))) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s thumbnail file also not there, need to regenerate") % (i + 1);
                        if (!skip_this_plate) {
                ...
                if (plate_data->no_light_thumbnail_file.empty()) {
                    ...
                    if (plate_data->no_light_thumbnail_file.empty() || (!boost::filesystem::exists(plate_data->no_light_thumbnail_file))) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format(
                                   "thumbnails stage: plate %1%'s no_light_thumbnail_file %2% also not there, need to regenerate") %
                ...
                if (plate_data->top_file.empty() || plate_data->pick_file.empty()) {
                    ...
                    if (plate_data->top_file.empty() || plate_data->pick_file.empty() ||
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("thumbnails stage: plate %1%'s top_file %2% also not there, need to regenerate") % (i + 1) %
                                   plate_data->top_file;
                        if (!skip_this_plate) {
                        ...
                        if (regenerate_thumbnails) {
                            BOOST_LOG_TRIVIAL(info)
                                << boost::format("thumbnails stage: plate %1%'s top_thumbnail file %2% cleared, need to regenerate") %
                                       (i + 1) % plate_data->top_file;
                            plate_data->top_file.clear();
                            plate_data->pick_file.clear();
                        } else
                            BOOST_LOG_TRIVIAL(info)
                                << boost::format("thumbnails stage: plate %1%'s top_thumbnail file exists, no need to regenerate") %
                                       (i + 1);
                    }
                }
            }
            ...
            if (need_regenerate_thumbnail || need_regenerate_no_light_thumbnail || need_regenerate_top_thumbnail) {
                        ...
                        if (!shader) {
                            ...
                            for (int i = 0; i < partplate_list.get_plate_count(); i++) {
                                ...
                                if (!plate_data->no_light_thumbnail_file.empty() &&
                                    ...
                                    if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                        BOOST_LOG_TRIVIAL(info) << boost::format("Line %1%: regenerate thumbnail, clear plate %2%'s "
                                                                                 "no_light_thumbnail_file path to empty.") %
                                                                       __LINE__ % (i + 1);
                                        plate_data->no_light_thumbnail_file.clear();
                                    } else
                                        BOOST_LOG_TRIVIAL(info)
                                    ...
                                    if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                        ...
                                        BOOST_LOG_TRIVIAL(info)
                                            << boost::format("plate %1%'s no_light thumbnail,finished rendering") % (i + 1);
                                    }
                                }
                                ...
                                if (need_create_no_light_group) {
                                    ...
                                    BOOST_LOG_TRIVIAL(info)
                                        << boost::format("plate %1%: add thumbnail data for no_light into group") % (i + 1);
                                }
                                ...
                                thumbnail.")%__LINE__%(i+1); part_plate->top_thumbnail_data.reset();
                                part_plate->pick_thumbnail_data.reset(); plate_data->top_file.clear(); plate_data->pick_file.clear();
                                    }
                                    else {
                                        plate_data->top_file = "valid_top";
                                        plate_data->pick_file = "valid_pick";
                                        BOOST_LOG_TRIVIAL(info) << boost::format("plate %1% has a valid top/pick thumbnail data, directly
                                using it")%(i+1);
                                    }
                                }
                                else*/
                                if ((!plate_data->top_file.empty() && (boost::filesystem::exists(plate_data->top_file))) &&
                                    ...
                                    if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                        BOOST_LOG_TRIVIAL(info) << boost::format("Line %1%: regenerate thumbnail, clear plate %2%'s "
                                                                                 "top/pick thumbnail file path to empty.") %
                                                                       __LINE__ % (i + 1);
                                        plate_data->top_file.clear();
                                        plate_data->pick_file.clear();
                                    } else
                                        BOOST_LOG_TRIVIAL(info)
                                            << boost::format(
                                                   "plate %1% has valid top/pick thumbnail extracted from 3mf, directly using it") %
                                                   (i + 1);
                                } else {
                                    ...
                                    if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                        ...
                                        BOOST_LOG_TRIVIAL(info)
                                            << boost::format("plate %1%'s top/pick thumbnail missed, need to regenerate, "
                                        ...
                                        if (skip_useless_pick && ((plate_object_count[i] <= 1) || (plate_object_count[i] > 64))) {
                                            ...
                                            case Slic3r::GUI::OpenGLManager::EFramebufferType::Ext: {
                                                ...
                                                Slic3r::GUI::GLCanvas3D::render_thumbnail_framebuffer_ext(*top_thumbnail, thumbnail_width,
                                                                                                          thumbnail_height,
                                                                                                          thumbnail_params, partplate_list,
                                                                                                          model.objects,
                                                                                                          glvolume_collection, colors_out,
                                                                                                          shader,
                                                                                                          Slic3r::GUI::Camera::EType::Ortho,
                                            ...
                                            plate_data->pick_file = "valid_pick";
                                            BOOST_LOG_TRIVIAL(info)
                                                << boost::format("plate %1%'s top_thumbnail,finished rendering") % (i + 1);
                                        }
                                    }
                                }
                ...
                for (int i = 0; i < partplate_list.get_plate_count(); i++) {
                    ...
                    if (skip_this_plate) {
                        ...
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format(
                                   "no need to generate: plate %1% has a valid thumbnail %2% extracted from 3mf, convert to data") %
                                   (i + 1) % plate_data->thumbnail_file;
                        int dec_ret = decode_png_to_thumbnail(plate_data->thumbnail_file, plate_data->plate_thumbnail);
                        if (!dec_ret) {
                            BOOST_LOG_TRIVIAL(info) << boost::format("decode png to mem sucess.");
            ...
            for (int i = 0; i < partplate_list.get_plate_count(); i++) {
                ...
                if (plate_bed_type == btDefault) {
                    ...
                    if (cur_bed_type) {
                        BOOST_LOG_TRIVIAL(info)
                            << boost::format("plate %1% bed type: %2%, set from global") % (i + 1) % cur_bed_type->serialize();
    ...
    bool CLI::setup(int argc, char** argv)
    {
        ...
        // Initialize with defaults BEFORE validation.
        // This ensures config is fully populated before we validate it.
        for (const t_optiondef_map* options :
             {&cli_actions_config_def.options, &cli_transform_config_def.options, &cli_misc_config_def.options})
            for (const t_optiondef_map::value_type& optdef : *options)
                m_config.option(optdef.first, true);

        set_data_dir(m_config.opt_string("datadir"));

        // NOW validate - config is fully initialized
        // Previously this was done before initialization, causing validation to fail on incomplete config
    ...
    void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
    {
        ...
        boost::nowide::cout << SLIC3R_APP_KEY << "-" << SoftFever_VERSION << ":" << std::endl
                            << "Usage: jus-prin [ OPTIONS ] [ file.3mf/file.stl ... ]" << std::endl
                            << std::endl
                            << "OPTIONS:" << std::endl;
    ...
    bool CLI::export_project(Model * model, std::string & path, PlateDataPtrs & partplate_data, std::vector<Preset*> & project_presets,
                             std::vector<ThumbnailData*> & thumbnails, std::vector<ThumbnailData*> & no_light_thumbnails,
                             std::vector<ThumbnailData*> & top_thumbnails, std::vector<ThumbnailData*> & pick_thumbnails,
                             std::vector<ThumbnailData*> & calibration_thumbnails, std::vector<PlateBBoxData*> & plate_bboxes,
                             const DynamicPrintConfig* config, bool minimum_save, int plate_to_export)
    {
