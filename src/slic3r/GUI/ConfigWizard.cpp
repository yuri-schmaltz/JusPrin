    ...
    namespace Slic3r
{
    ...
    struct PrinterPickerEvent : public wxEvent
    {
        ...
        PrinterPickerEvent(
            wxEventType eventType, int winid, std::string vendor_id, std::string model_id, std::string variant_name, bool enable)
    ...
    PrinterPicker::PrinterPicker(wxWindow*            parent,
        ...
        : wxPanel(parent), vendor_id(vendor.id), width(0)
    {
        ...
        for (const auto& model : models) {
            ...
            if (!load_bitmap(GUI::from_u8(Slic3r::data_dir() + "/vendor/" + vendor.id + "/" + model.id + "_thumbnail.png"), bitmap,
                             bitmap_width)) {
                if (!load_bitmap(GUI::from_u8(Slic3r::resources_dir() + "/profiles/" + vendor.id + "/" + model.id + "_thumbnail.png"),
                                 bitmap, bitmap_width)) {
                    BOOST_LOG_TRIVIAL(warning)
                        << boost::format("Can't find bitmap file `%1%` for vendor `%2%`, printer `%3%`, using placeholder icon instead") %
                               (Slic3r::resources_dir() + "/profiles/" + vendor.id + "/" + model.id + "_thumbnail.png") % vendor.id %
                               model.id;
    ...
    static void focus_event(wxFocusEvent& e, wxTextCtrl* ctrl, double def_value)
    {
        ...
        double val    = 0.0;
        auto   result = Slic3r::Utils::parse_double_safe(str, 0.0,
                                                         std::numeric_limits<double>::max()); // Assuming min=0, max=infinity for now
        if (!result.success) {
            if (val == 0.0)
                val = def_value;
    ...
    void ConfigWizard::priv::init_dialog_size()
    {
        ...
        const int width_hint = index->GetSize().GetWidth() +
                               90 *
                                   em() /*std::max(90 * em(), (only_sla_mode ? page_msla->get_width() : page_fff->get_width()) + 30 * em())*/
    ...
    //
    // In case only_for_model_id is set, then the test is done for that particular printer model only, and the default materials are
    // installed silently. Otherwise the user is quieried whether to install the missing default materials or not.
    //
                                          ...
                                          bool&                apply_keeped_changes)
    {
        ...
        auto              get_preferred_printer_technology = [enabled_vendors, suppress_sla_printer](const std::string& bundle_name,
                                                                                        const Bundle&      bundle) {
        ...
        /*if (snapshot && ! take_config_snapshot_cancel_on_error(*app_config, snapshot_reason, "", _u8L("Do you want to continue changing
           the configuration?"))) return false;*/
        ...
        auto        get_preferred_printer_model = [enabled_vendors, enabled_vendors_old,
                                            preferred_pt](const std::string& bundle_name, const Bundle& bundle, std::string& variant) {
            const auto config = enabled_vendors.find(bundle_name);
            if (config == enabled_vendors.end())
                return std::string();
            for (const auto& model : bundle.vendor_profile->models) {
        ...
        if (!check_unsaved_preset_changes) {
            if ((check_unsaved_preset_changes = !first_added_filament.empty() || !first_added_sla_material.empty())) {
                ...
                ((check_unsaved_preset_changes = is_filaments_changed || is_sla_materials_changed)) { header = is_filaments_changed ?
                _L("Some filaments were uninstalled.") : _L("Some SLA materials were uninstalled."); if
