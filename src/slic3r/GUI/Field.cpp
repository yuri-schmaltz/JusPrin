    ...
    case coFloat: {
        ...
        if (str == dec_sep)
            ...
            if (is_na_value)
                ...
                auto parse_result = Slic3r::Utils::parse_double_safe(str);
                if (!parse_result.success) {
                    if (!check_value) {
                        m_value.clear();
                        break;
                    }
                    show_error(m_parent, from_u8(parse_result.error_message));
    ...
    case coFloatOrPercent: {
        if (m_opt.type == coFloatOrPercent && !str.IsEmpty() && str.Last() != '%') {
            ...
            if (!parse_result.success) {
                ...
                val = parse_result.value;
                if (((m_opt.sidetext.rfind("mm/s") != std::string::npos && val > m_opt.max) ||
            ...
            m_value = into_u8(str);
            break;
        } else { // This else block handles the case where str.Last() == '%' or str.IsEmpty() for coFloatOrPercent
            // Or it's a coString/coStrings type.
            // The following logic for thumbnails, sparse_infill_rotate_template, extra_solid_infills
            // should apply to coString/coStrings and potentially to coFloatOrPercent when it's a percentage.
            // However, the original code structure suggests these are general string validations.
            // Given the instruction "Close the else block properly after previous edit",
            // and the previous edit context, this `else` block is likely intended to contain
            // the string-specific validations that follow, when the initial `if` condition (non-percentage float) is false.
            // This means the `m_value = into_u8(str); break;` at the end of the `if` block above
            // should only apply to that specific float parsing path.
            // The `m_value = into_u8(str); break;` at the end of this `else` block (or the case)
            // would be the general assignment for string types.
        }
...
{
    if (!is_defined_input_value<wxTextCtrl>(text_ctrl(),
                                            m_opt.type)) { // BBS
...
void ColourPicker::draw_bmp_btn(wxColourPickerCtrl* field, wxColour color)
{
    ...
    auto create_bitmap = [btn](const wxColour& picker_color, const wxColour& bg_color, bool focus) -> wxBitmap {
        ...
        dc2.SetPen(focus ? wxPen(wxColour(StateColor::darkModeColorFor(wxColour("#694b7c"))), 1) : *wxTRANSPARENT_PEN);
...
void PointCtrl::BUILD()
{
    ...
    const wxSize  field_size((m_opt.width >= 0 ? m_opt.width : def_width_wider()) * m_em_unit,
                             -1); // ORCA match width with other components
